...
export async function GET() {
  ...
  const [products, bespoke, orders, subscribers, settings] = await Promise.all([
    ...
    db.prepare(`SELECT id, source_enquiry_id AS sourceEnquiryId, user_id AS userId, order_number AS orderNumber, client_name AS clientName, client_contact AS clientContact,
      items_json AS itemsJson, total_etb AS totalEtb, payment_method AS paymentMethod, payment_status AS paymentStatus, shipping_address AS shippingAddress, status, created_at AS createdAt,
