...
type OrderRow = { id: string; sourceEnquiryId: string | null; userId: string | null; orderNumber: string; clientName: string; clientContact: string; itemsJson: string; totalEtb: number; paymentMethod: string; paymentStatus: string; shippingAddress: string; status: string; createdAt: string };
...
function Orders({ rows, act }: { rows: OrderRow[]; act: (payload: Record<string, unknown>, success: string) => Promise<boolean> }) {
  ...
  return <div className="admin-card table-card"><div className="table-toolbar"><label><span aria-hidden="true">⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search orders" aria-label="Search orders" /></label></div><div className="table-scroll"><table><thead><tr><th>Order</th><th>Client</th><th>Items</th><th>Total</th><th>Payment</th><th>Date</th><th>Fulfilment</th></tr></thead><tbody>{visible.map((item) => <tr key={item.id}><td><strong>{item.orderNumber}</strong><small>{item.userId ? "Authenticated" : item.sourceEnquiryId ? "Bespoke" : "Guest checkout"}</small></td><td>{item.clientName}<small>{readContact(item.clientContact)}</small></td><td>{readItemCount(item.itemsJson)}</td><td>{money(item.totalEtb)} ETB</td><td><span className="payment-method">{paymentName(item.paymentMethod)}</span><select className="mini-select" value={item.paymentStatus || "pending"} onChange={(event) => void act({ action: "order.update", id: item.id, status: item.status, paymentStatus: event.target.value }, `Payment for ${item.orderNumber} updated.`)}><option value="pending">Pending</option><option value="paid">Paid</option><option value="failed">Failed</option><option value="refunded">Refunded</option></select></td><td>{date(item.createdAt)}</td><td><select className="mini-select" value={item.status} onChange={(event) => void act({ action: "order.update", id: item.id, status: event.target.value, paymentStatus: item.paymentStatus }, `Order ${item.orderNumber} updated.`)}><option value="new">New</option><option value="confirmed">Confirmed</option><option value="in_progress">In progress</option><option value="shipped">Shipped</option><option value="complete">Complete</option><option value="refunded">Refunded</option></select></td></tr>)}</tbody></table></div>{!visible.length && <Empty icon="⌑" title="No client orders yet" body="Storefront purchases and confirmed bespoke orders will appear here." />}</div>;
}
...
function readContact(value: string) { try { const parsed = JSON.parse(value) as { email?: string; phone?: string }; return [parsed.email, parsed.phone].filter(Boolean).join(" · ") || value; } catch { return value; } }
function paymentName(value: string) { return ({ telebirr: "Telebirr", bank_transfer: "Bank transfer", cash_on_delivery: "Pay on delivery" }[value] ?? value?.replaceAll("_", " ") ?? "Bank transfer"); }
